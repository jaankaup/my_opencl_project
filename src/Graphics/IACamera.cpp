...
InteractiveCamera::InteractiveCamera()
{
	...
	resolution = glm::vec2(512, 512);  // width, height
	fov = glm::vec2(40, 40);
}
...
void InteractiveCamera::strafe(float m){
	glm::vec3 strafeAxis = cross(viewDirection, glm::vec3(0, 1, 0));
	strafeAxis = glm::normalize(strafeAxis); // strafeAxis.normalize()
...
void InteractiveCamera::buildRenderCamera(Camera* renderCamera){
	...
	//glm::vec3 eyePosition = centerPosition; // rotate camera from stationary viewpoint
