...
void Window::renderImgui()
{
    ...
    static bool show_density_values = false;
    static ImVec4 clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f);
        ...
        {
          if (ImGui::BeginMenu("AAAA")) 
          {
            ImGui::EndMenu();          
          }
          ImGui::EndMenuBar();
        } // END MENUBAR
          if (ImGui::CollapsingHeader("Juhuuu")) 
          {
              if (ImGui::TreeNode("Global attributes")) 
              {
                auto property = glob_manager->get<Program::BoolProperty>("show_density");
                show_density_values = property->get();
                ImGui::Checkbox("Show density values", &show_density_values);      // Edit bools storing our window open/close state
                property->set(show_density_values);
                ImGui::TreePop();
              }

              if (ImGui::TreeNode("Hello, world!")) 
              {
                static float f = 0.0f;
